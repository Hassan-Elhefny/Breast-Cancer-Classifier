...
#show final result of classifier
st.header("Classifier Result")
pred = extra_trees_clf.predict(selected_features_df)
str_pred = []
if(pred[0] == 0):
    str_pred.append("benign")
else:
    str_pred.append('malignant')
pred_df = pd.DataFrame(str_pred,columns=['Predicted'])
st.write(pred_df)

#show probabilities of classifier
st.header("probabilities")
probabilities = extra_trees_clf.predict_proba(selected_features_df)
